...
class golog:
    ...
    def addCircle(self,name,event):
        ...
        self.category.hom(object,object)[0].graphics = ptc.FancyArrowPatch(posA=object.graphics.get_center(),posB=object.graphics.get_center()) #set empty arrow for identity
        ax.add_patch(circ)
        fig.canvas.draw()
...
class circle:
    ...
    def click(self,event):
        ...
        self.attachedMors = self.golog.category.getAttachedMors(self.object)
        print([f.pr for f in self.attachedMors])
    ...
    def dragArrows(self):
        for f in self.attachedMors:
            f.graphics.set_positions(f.dom.graphics.get_center(),f.codom.graphics.get_center())
