...
class export_data():
    def __init__(self,golog, old_simplex):
        ...
        if old_simplex.math_data.type == 'golog':
            self.exported_math_data = Math_Data(type = 'exported golog', math_data = gexport(old.simplex.math_data()))
    ...
    def transform(self):
        if self.exported_math_data.type == 'exported golog':
            return Math_Data(type = 'golog', math_data = sSet_to_golog(self.exported_math_data()))
...
def gexport(golog,location_string):
    ...
    with open(location_string,'wb') as file:
        pickle.dump(old_to_new.codom,file)
...
def sSet_to_golog(base, sSet):
    ...
    def setupSimplex(simplex):
        print('setting up simplex '+ simplex.label)
        #check if faces are in golog
        for face in simplex.faces:
            if face not in golog.sSet.rawSimps:
                setupSimplex(face)
        print(simplex.math_data().graphics_kwargs)
        simp = golog.add(simplex, label = simplex.label, math_data = simplex.math_data().transform(),**simplex.math_data().graphics_kwargs)
